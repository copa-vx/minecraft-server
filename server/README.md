...
A vanilla Minecraft: Java Edition **dedicated** server, packaged as a
[Celaut](https://github.com/celaut-project/nodo) service (microVM): `.service/`
(Dockerfile, `service.json`, `pack_config.json`), `service/` (the entrypoint),
`tests/`, and this README. It has no display of its own — the game runs entirely
in whichever client connects to it.

This is one of two architectures in this repository; see the [top-level
README](../README.md) for why there are two and which one fits what you want to
run. The other, [`client-stream/`](../client-stream/), runs a Minecraft client
*inside* the microVM too and shows it to you as a video stream — no Minecraft
installation of your own required, at the cost of needing one.
...
could override at launch would make it mean less. It is pinned to the same version
as `client-stream/`, since the two talk the same protocol.
...
no network entry to be honoured at all. `client-stream/`, the sibling architecture,
needs none of this either way: its bundled server always runs with
`online-mode=false`.
...
The tests live at the top of the repository and cover both architectures; see
...
Built and run: the image builds, the server boots to `Done`, writes
`server.properties` and its logs correctly, and stops cleanly on `SIGTERM`,
confirmed on 2026-09-20. Not yet confirmed: an actual Minecraft client
connecting to it over the network — see `NODE-REQUIREMENTS.md` for what to
check first if that is where it breaks.
