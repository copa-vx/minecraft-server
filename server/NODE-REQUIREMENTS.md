...
sibling, [`client-stream/`](../client-stream/).
...
Built and run, not just specified — in a plain Docker container on
`linux/arm64`, not yet an actual `nodo` microVM: the image builds, the server
reaches `Done` with a freshly generated world, and stops cleanly on `SIGTERM`
— confirmed 2026-09-20. Not confirmed: a real Minecraft client actually
connecting, or anything specific to running inside `nodo` itself.
...
**Found by actually running it, and fixed:** `write_configuration` and the
`mkdir -p`s before it run as root, so everything they create under `/data` —
`server.properties`, `logs/` — was root-owned by the time the JVM, running as
`minecraft`, tried to write into it. The server still reached `Done` regardless
(Mojang's own log4j and properties-rewrite failures are non-fatal, so the
symptom was silent: no `logs/latest.log`, and every later
`server.properties` rewrite failing quietly) — which is exactly the class of
bug this architecture is otherwise careful to turn loud. `main()` now chowns
`/data` to `minecraft` right before dropping to it.
